...
export default function RegisterPage() {
  ...
  const strengthColor = ['', '#ffffff', '#ffffff', '#ffffff'][strength];
  ...
  return (
    <div style={{ paddingTop: 70, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1.5rem' }}>
      <div style={{ position: 'fixed', inset: 0, background: 'radial-gradient(ellipse 80% 60% at 50% 20%, rgba(255,255,255,0.1) 0%, transparent 70%)', pointerEvents: 'none', zIndex: 0 }} />
      ...
      <div style={{ width: '100%', maxWidth: 460, position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <Link href="/" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem' }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: 'linear-gradient(135deg,#ffffff,#ffffff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem' }}>⚡</div>
            <span style={{ fontFamily: 'var(--font-outfit)', fontWeight: 700, fontSize: '1.4rem', color: '#fff' }}>SHOP</span>
          </Link>
          <h1 style={{ fontFamily: 'var(--font-outfit)', fontSize: '2rem', fontWeight: 800, color: '#fff', marginBottom: '0.5rem' }}>Create account</h1>
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.9rem' }}>Join SHOP and start shopping today</p>
        </div>

        <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 24, padding: '2rem', boxShadow: '0 32px 64px rgba(0,0,0,0.4)' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div>
              ...
              <input className="input" type="password" value={form.confirm} onChange={update('confirm')} placeholder="Re-enter password" required style={{ borderColor: form.confirm && form.confirm !== form.password ? 'rgba(255,255,255,0.5)' : undefined }} />
              {form.confirm && form.confirm !== form.password && <p style={{ color: '#ffffff', fontSize: '0.75rem', marginTop: '0.3rem' }}>Passwords do not match</p>}
            </div>
          ...
          <p style={{ textAlign: 'center', marginTop: '1.25rem', color: 'rgba(255,255,255,0.35)', fontSize: '0.75rem' }}>By signing up, you agree to our <a href="#" style={{ color: '#ffffff' }}>Terms</a> and <a href="#" style={{ color: '#ffffff' }}>Privacy Policy</a></p>
        </div>

        <p style={{ textAlign: 'center', marginTop: '1.5rem', color: 'rgba(255,255,255,0.4)', fontSize: '0.875rem' }}>
          Already have an account?{' '}
          <Link href="/auth/login" style={{ color: '#ffffff', textDecoration: 'none', fontWeight: 600 }}>Sign in →</Link>
        </p>
      </div>
    </div>
  );
