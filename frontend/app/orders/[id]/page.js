...
const STATUS_CONFIG = {
  placed: { color: '#ffffff', icon: '📋', label: 'Order Placed' },
  confirmed: { color: '#ffffff', icon: '✅', label: 'Confirmed' },
  processing: { color: '#ffffff', icon: '⚙️', label: 'Processing' },
  shipped: { color: '#ffffff', icon: '🚚', label: 'Shipped' },
  delivered: { color: '#ffffff', icon: '📦', label: 'Delivered' },
  cancelled: { color: '#ffffff', icon: '✕', label: 'Cancelled' },
  returned: { color: '#ffffff', icon: '↩️', label: 'Returned' },
};
  ...
  return (
    <div style={{ paddingTop: 70, minHeight: '100vh' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
        {/* Success Banner */}
        {isSuccess && (
          <div style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 16, padding: '1.5rem', marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', flexShrink: 0 }}>🎉</div>
            <div>
              <div style={{ fontWeight: 700, color: '#ffffff', marginBottom: '0.25rem' }}>Order Placed Successfully!</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem' }}>
          ...
          {['placed', 'confirmed'].includes(order.orderStatus) && (
            <button onClick={handleCancel} disabled={cancelling} style={{ padding: '0.6rem 1.25rem', borderRadius: 10, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.3)', color: '#ffffff', cursor: 'pointer', fontWeight: 600, fontSize: '0.875rem' }}>
        ...
        {/* Progress Tracker */}
        {!isCancelled && (
          <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, padding: '1.75rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', top: 18, left: '10%', height: 2, width: `${Math.max(0, stepIdx) * (80 / (STATUS_STEPS.length - 1))}%`, background: 'linear-gradient(90deg,#ffffff,#cccccc)', transition: 'width 0.5s ease', zIndex: 1 }} />
              {STATUS_STEPS.map((status, i) => {
                ...
                return (
                  <div key={status} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', position: 'relative', zIndex: 2 }}>
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: done ? 'linear-gradient(135deg,#ffffff,#cccccc)' : '#0a0a0a', border: `2px solid ${done ? '#ffffff' : 'rgba(255,255,255,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.9rem', transition: 'all 0.3s' }}>{done ? (i === stepIdx ? cfg.icon : '✓') : cfg.icon}</div>
                    <span style={{ fontSize: '0.7rem', fontWeight: done ? 600 : 400, color: done ? '#ffffff' : 'rgba(255,255,255,0.35)', textAlign: 'center' }}>{cfg.label}</span>
                  </div>
                );
              })}
            </div>
            {order.trackingNumber && <p style={{ marginTop: '1rem', fontSize: '0.875rem', color: 'rgba(255,255,255,0.5)' }}>Tracking #: <span style={{ color: '#ffffff', fontWeight: 600 }}>{order.trackingNumber}</span></p>}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', alignItems: 'start' }}>
          {/* Items */}
          <div>
            <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, padding: '1.5rem', marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {order.items.map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, color: '#ffffff', fontSize: '0.875rem', marginBottom: '0.2rem' }}>{item.name}</div>
          ...
          {/* Summary */}
          <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1rem' }}>
              {[
                ...
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                  ...
                  <span style={{ color: l === 'Discount' ? '#ffffff' : '#ffffff' }}>{v}</span>
                </div>
              ))}
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {[
                ...
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem' }}>
                  ...
                  <span style={{ color: '#ffffff', fontWeight: 500, textTransform: 'capitalize' }}>{v}</span>
                </div>
              ))}
            </div>
