  ...
  return (
    <div style={{ paddingTop: 70, minHeight: '100vh' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2.5rem', background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, padding: '1.75rem' }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg,#ffffff,#ffffff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.75rem', fontWeight: 800, color: '#fff', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
              <span style={{ padding: '0.2rem 0.6rem', borderRadius: 6, background: user.role === 'admin' ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.12)', color: user.role === 'admin' ? '#ffffff' : '#ffffff', fontSize: '0.75rem', fontWeight: 600, textTransform: 'capitalize' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '1.5rem', alignItems: 'start' }}>
          {/* Sidebar */}
          <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, overflow: 'hidden' }}>
            {TABS.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1.25rem', background: activeTab === tab.id ? 'rgba(255,255,255,0.15)' : 'transparent', border: 'none', borderLeft: `3px solid ${activeTab === tab.id ? '#ffffff' : 'transparent'}`, color: activeTab === tab.id ? '#ffffff' : 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 600, textAlign: 'left', transition: 'all 0.2s' }}>
                {tab.icon} {tab.id.charAt(0).toUpperCase() + tab.id.slice(1)}
              </button>
            ))}
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', margin: '0.25rem 0' }} />
            <button onClick={logout} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1.25rem', background: 'transparent', border: 'none', color: '#ffffff', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 600, textAlign: 'left' }}>
              🚪 Logout
            </button>
          </div>

          {/* Content */}
          <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, padding: '1.75rem' }}>
            {activeTab === 'profile' && (
                    ...
                    {user.addresses.map((addr) => (
                      <div key={addr._id} style={{ padding: '1.125rem', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                          ...
                          {addr.isDefault && <span style={{ fontSize: '0.75rem', color: '#ffffff', background: 'rgba(255,255,255,0.1)', padding: '0.15rem 0.5rem', borderRadius: 6, fontWeight: 600 }}>Default</span>}
                        </div>
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '1.25rem' }}>
                  ...
                  <form onSubmit={addAddress} style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.875rem', color: 'rgba(255,255,255,0.6)' }}>
                      <input type="checkbox" checked={addressForm.isDefault} onChange={(e) => setAddressForm((p) => ({ ...p, isDefault: e.target.checked }))} style={{ accentColor: '#ffffff' }} />
