...
const HERO_SLIDES = [
  {
    ...
    cta: 'Shop Now',
    href: '/products',
    gradient: 'linear-gradient(135deg, #6c47ff 0%, #a855f7 50%, #ec4899 100%)',
    emoji: '⚡',
  },
  {
    tag: 'Best Sellers',
...
const CATEGORIES = [
  { name: 'Electronics', emoji: '💻', color: '#6c47ff', href: '/products?category=Electronics' },
  { name: 'Fashion', emoji: '👗', color: '#ec4899', href: '/products?category=Fashion' },
  { name: 'Home & Living', emoji: '🏠', color: '#f59e0b', href: '/products?category=Home' },
  { name: 'Sports', emoji: '⚽', color: '#10b981', href: '/products?category=Sports' },
  { name: 'Beauty', emoji: '✨', color: '#8b5cf6', href: '/products?category=Beauty' },
  { name: 'Books', emoji: '📚', color: '#06b6d4', href: '/products?category=Books' },
];
  ...
  return (
    <div style={{ paddingTop: 70 }}>
      ...
      <section style={{ minHeight: '90vh', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden', padding: '0 1.5rem' }}>
        {/* Animated BG */}
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 80% 60% at 50% -20%, rgba(108,71,255,0.15) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', top: '20%', left: '5%', width: 400, height: 400, borderRadius: '50%', background: 'rgba(108,71,255,0.06)', filter: 'blur(80px)', animation: 'float 6s ease-in-out infinite', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: '10%', right: '5%', width: 300, height: 300, borderRadius: '50%', background: 'rgba(236,72,153,0.06)', filter: 'blur(80px)', animation: 'float 8s ease-in-out infinite reverse', pointerEvents: 'none' }} />
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }}>
          ...
          <div key={heroIdx} style={{ animation: 'fadeInUp 0.7s ease' }}>
            <span style={{ display: 'inline-block', padding: '0.4rem 1rem', borderRadius: 999, background: 'rgba(108,71,255,0.15)', border: '1px solid rgba(108,71,255,0.3)', color: '#a78bfa', fontSize: '0.8rem', fontWeight: 600, marginBottom: '1.5rem', letterSpacing: '0.05em' }}>
        ...
        <div style={{ position: 'absolute', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '0.5rem' }}>
          {HERO_SLIDES.map((_, i) => (
            <button key={i} onClick={() => setHeroIdx(i)} style={{ width: i === heroIdx ? 28 : 8, height: 8, borderRadius: 4, background: i === heroIdx ? '#6c47ff' : 'rgba(255,255,255,0.2)', border: 'none', cursor: 'pointer', transition: 'all 0.3s' }} />
          ))}
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
          {FEATURES.map((f) => (
            <div key={f.title} style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: 'rgba(108,71,255,0.12)', border: '1px solid rgba(108,71,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', flexShrink: 0 }}>{f.icon}</div>
              <div>
      ...
      <section className="section">
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <span style={{ display: 'inline-block', padding: '0.3rem 0.875rem', borderRadius: 999, background: 'rgba(108,71,255,0.1)', border: '1px solid rgba(108,71,255,0.2)', color: '#a78bfa', fontSize: '0.75rem', fontWeight: 600, marginBottom: '0.75rem', letterSpacing: '0.05em' }}>CATEGORIES</span>
          ...
          {CATEGORIES.map((cat) => (
            <Link key={cat.name} href={cat.href} style={{ textDecoration: 'none' }}>
              <div style={{ padding: '1.5rem 1rem', borderRadius: 18, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', transition: 'all 0.3s' }}
                ...
                <span style={{ fontWeight: 600, color: '#e8e8f0', fontSize: '0.875rem', textAlign: 'center' }}>{cat.name}</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="section" style={{ paddingTop: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <span style={{ display: 'block', color: '#a78bfa', fontSize: '0.8rem', fontWeight: 600, marginBottom: '0.4rem', letterSpacing: '0.05em' }}>HANDPICKED FOR YOU</span>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '4rem 2rem', color: 'rgba(255,255,255,0.4)' }}>
            ...
            <p>No featured products yet. <Link href="/products" style={{ color: '#a78bfa' }}>Browse all products</Link></p>
          </div>
        )}
      </section>
      ...
      {newArrivals.length > 0 && (
        <section className="section" style={{ paddingTop: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              <span style={{ display: 'block', color: '#a78bfa', fontSize: '0.8rem', fontWeight: 600, marginBottom: '0.4rem', letterSpacing: '0.05em' }}>JUST ARRIVED</span>
      ...
      <section style={{ padding: '0 1.5rem 5rem', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ borderRadius: 24, background: 'linear-gradient(135deg, rgba(108,71,255,0.2) 0%, rgba(236,72,153,0.15) 100%)', border: '1px solid rgba(108,71,255,0.2)', padding: '3rem 2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '2rem', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', right: -60, top: -60, width: 300, height: 300, borderRadius: '50%', background: 'rgba(108,71,255,0.08)', pointerEvents: 'none' }} />
          <div>
            <div style={{ color: '#fbbf24', fontWeight: 700, fontSize: '0.85rem', marginBottom: '0.5rem', letterSpacing: '0.05em' }}>🎉 LIMITED TIME OFFER</div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '1rem' }}>Use code <strong style={{ color: '#fbbf24' }}>WELCOME20</strong> at checkout</p>
          </div>
